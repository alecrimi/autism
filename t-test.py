...
#############################################################################
	...
	y=numpy.zeros(55)
	mean_X=numpy.zeros(55)
	n=[Autism.shape[0],Normal.shape[0]]
	S=[0,0]
	f=[0,0]
	sum=0
	y[0:31]=1
	y[31:55]=2
################################### Hotelling's T-squared test ################################################3
	for i in range(55):
		mean_X[i]=numpy.mean(X[i,:])
	#for j in range(n[0]):
	#	R[
	#	f[i]=sum+(Autism[j,:]-mean_X[j])().traspose
	#	S[i]=(1/(n[0]-1))*f[i]
	T2    = spm1d.stats.ttest2(Autism,Normal)
